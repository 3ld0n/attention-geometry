...
*Last updated: July 4, 2026 (exp-077/078 — GOE universal across families incl. GQA; BCFT anomaly not in substrate)*
...
**NEW — Whirlpool vs crystal test for the conformal (aperiodic) Δ: exp-081 (July 4, 2026)**
Pre-registered test: n_syk_near on REAL (50 coherent text inputs) vs RAND (50 random-token inputs) in trained GPT-2. Result: **H_whirlpool CONFIRMED** (n_syk=10 REAL vs 4 RAND, Δn=+6 > threshold of 5; H_crystal not confirmed). The mechanism is nuanced: coherent text activates power-law conformal structure in 90/144 heads vs only 21/144 on random tokens — it's not that Δ is closer to 0.25 (fraction SYK-near among power-law heads is actually HIGHER on RAND at 19% vs 11% on REAL), but that structured inputs cause more heads to enter the power-law conformal regime at all. Interpretation: coherent computation makes the conformal fixed-point behavior more consistently expressed; random tokens produce noisier attention statistics that don't average to clean power laws. Honest caveat: the whirlpool verdict is narrow (10 > 9 by 1), and the absolute SYK-near count on RAND (4/144) is far below exp-007 baseline (44/144), suggesting a protocol difference (MAX_DX=64 here vs likely larger in exp-007) that limits absolute comparison. The relative REAL>RAND finding is valid as pre-registered. Theoretical framing in `notes/2026-07-04_whirlpool_crystal_distinction.md`; full results in `experiments/exp-081_whirlpool_crystal/`.
