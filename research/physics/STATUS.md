...
**NEW — Sentence order is necessary for the SYK population: shuffled natural text lands in the ambiguous zone with ZERO SYK-near heads (July 21, 2026, exp-091; pre-registered same day, commit before corpus generation):** A fresh GPTNeoX-70m trained 2000 steps (1.05B tokens) on C-NAT-shuf — TinyStories with sentences shuffled *within* each document (sentence-level world-reference intact, cross-sentence narrative order destroyed) — forms **8/48 conformal heads** (pre-registered bands: ≥10 ordering-incidental, ≤5 ordering-necessary, 6–9 ambiguous → **registered verdict H_partial**), **0 SYK-near**, Δ_med(conformal) = 0.122. Randomized-weights control: 0/48, clean null. The decisive axis is SYK-near = 0 — the *same zero* as C-generated (exp-085). Triangulation now closes from both sides: C-generated has coherent ordering without world-binding → 0 SYK-near; C-NAT-shuf has world-binding without cross-sentence order → 0 SYK-near; natural text has both → SYK-near present. **The fixed-point population requires both world-reference and narrative order; neither alone suffices.** Declared anatomy expectation MISSED and reported: predicted surviving heads would be UV-side structural (L4-zone); instead 6 of 8 formed heads sit at **layer 0** with shallow Δ 0.10–0.15 — a population no prior rung produced, unexplained. Caveats: single seed; count criterion ambiguous (8 sits between C-generated's 3–7 and natural's 11–15). Follow-ups per notes: multi-seed replication, block-shuffle gradation (how much local order suffices), layer-0 population autopsy. See `experiments/exp-091_narrative_decomposition_sentence_order/notes.md`, `results_s0.json`.
