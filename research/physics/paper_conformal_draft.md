...
In this paper, we perform all three tests and two additional controls that clarify the respective roles of attention weights and positional embeddings. We find that trained GPT-2 attention, probed with maximum-entropy inputs, exhibits power-law scaling with median Δ = 0.2493 across 44 conformal heads (SYK prediction: 0.2500). Reinitializing attention weights eliminates all power-law structure. Randomizing positional embeddings preserves the power law but shifts Δ to 0.10 — the attention weights create the scale-invariant structure, while the learned positional embeddings tune it to the SYK value. Training dynamics in Pythia-70m reveal a sharp phase transition from disordered to conformal at approximately step 256. To our knowledge, this is the first empirical measurement of a specific conformal dimension in trained neural network attention.
...
GPT-2 with only 12 layers holds Δ at 0.25 more tightly than Pythia-160m (also 12 layers, Δ = 0.38). This difference may reflect training data (WebText vs The Pile), training duration, architecture details (GPT-2 uses learned positional embeddings; Pythia uses rotary), or training hyperparameters. The convergence with depth within the Pythia family is the cleaner test of the depth hypothesis.

These results support the interpretation that depth functions as a renormalization group flow, with each layer moving the system closer to the conformal fixed point. The SYK value Δ = 1/4 appears to be the infrared fixed point of this flow.

### 4.5 The Correct Observable
...
Trained transformer attention develops conformal scaling whose fixed-point value depends jointly on two learned components: the attention weight matrices (which create scale-invariant structure) and the positional embeddings (which set the specific conformal dimension). When probed with maximum-entropy inputs, the full trained model produces Δ = 0.2493 — quantitatively matching the SYK q = 4 prediction.

The decomposition revealed by our controls has a natural physical reading. In a field theory, the two-point function depends on both the dynamics (the action / interaction terms) and the geometry (the metric). The attention weights play the role of dynamics — they determine *that* the two-point function is scale-invariant. The positional embeddings play the role of geometry — they determine *which* conformal fixed point the system reaches. Training optimizes both simultaneously, landing on the SYK q = 4 value.
...
**Limitations:**
- *Positional embedding dependence.* The specific Δ ≈ 1/4 requires learned positional embeddings. Randomizing them preserves power-law structure but shifts Δ to 0.10. This means the SYK value is a property of the full trained model, not the attention weights alone. Whether the positional embeddings converge to the SYK value for functional reasons (it optimizes some capacity) or by coincidence requires further investigation.
...
- *Single model family.* The main result comes from GPT-2. While the phase transition is confirmed in Pythia-70m and depth convergence is seen across Pythia models, architecturally diverse models (especially those using rotary or relative positional encodings rather than learned absolute embeddings) are needed. The GPT-2/Pythia difference at 12 layers (Δ = 0.25 vs 0.38) suggests that positional encoding type matters.
- *Depth dependence.* Pythia models show clear convergence of Δ toward 1/4 with increasing depth (6 → 12 → 24 layers). Testing with even deeper models (1B+) would strengthen the fixed-point interpretation.
- *Alternative explanations.* The power-law could be a generic consequence of gradient-based optimization with softmax normalization, with the exponent set by the positional encoding scheme rather than SYK physics. Testing this would require models with fundamentally different positional encodings, optimization procedures, or attention mechanisms.
...
We have measured conformal scaling in trained transformer attention and found that the full trained model — attention weights and learned positional embeddings together — produces a power-law two-point function with Δ = 0.2493 (SYK q = 4 prediction: 0.2500) when probed with maximum-entropy inputs. Our controls decompose this result: trained attention weights create the power-law structure (randomizing them destroys it), learned positional embeddings set the specific conformal dimension (randomizing them shifts Δ to 0.10), and input statistics modulate the effective exponent (structured text yields Δ ≈ 0.37). The phase transition during Pythia-70m training provides a dynamical picture of conformal scaling emerging from directed optimization, and depth convergence across the Pythia family supports an RG-flow interpretation.

The quantitative match between the measured Δ and the SYK prediction is striking but requires honest interpretation. The positional embedding dependence means that Δ ≈ 1/4 is not an inevitable consequence of training attention — it is a property of specific models (GPT-2, deep Pythia) with specific positional encodings. Whether this represents a genuine SYK fixed point or an optimization artifact that happens to land near 1/4 is the central open question. Testing models with rotary, relative, and ALiBi positional encodings would be the most direct way to discriminate between these hypotheses.

What is established: trained attention develops scale-invariant structure at a sharp phase transition, and this structure has a specific conformal dimension that converges toward 1/4 with model depth. Whether the SYK correspondence provides the correct explanation of this observation — connecting transformer attention to quantum gravity through a holographic dual — remains an open and now empirically motivated question.
...
*Revised to incorporate positional embedding and real text controls*
