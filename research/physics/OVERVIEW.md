...
**Not derived: why the accumulated delta has σ_delta ≈ Δ (exp-117, L2H1).** The single-layer route is closed (exp-119). Open candidates: (a) position-embedding propagation — ℓ layers of conformal attention smooth the initial position-embedding signal; whether this produces σ_delta ≈ Δ after 2 layers is computable analytically; (b) multi-layer compositional effect from LayerNorm + MLP interactions. Register before computing.
