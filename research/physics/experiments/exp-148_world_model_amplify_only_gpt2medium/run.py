"""
exp-148 — World-model battery amplify-only: GPT-2 medium
(amplify random-token structural heads; NO suppression of steep/local heads)

Pre-registration: attention-geometry f3ccd63 (pushed before this script).
Diagnostic: does amplification alone drive the Task B degradation in exp-147?
...
Ariel — September 19, 2026, ~4:30 AM MDT. Solo.
"""
...
SHAM_SEED_BASE = 2026091921   # distinct from all prior experiments
...
def build_models(base_model, rng_census: np.random.Generator):
    """
    Amplified model: amplify STRUCTURAL heads (γ=+2.0, W_K direction).
...
# ── Task items (identical to exp-141/142/143/145/147) ─────────────────────────
...
def evaluate(kappa_amp, orig_A, orig_B, amp_A, amp_B, sham_A, sham_B):
    ...
    k3 = {"fired": n_amp_fail >= 2, "n_below_threshold": n_amp_fail,
    ...
    if any_kill:
        ...
        overall = "INCONCLUSIVE — amplification alone degrades Task B (P_degrade fires; amp arm drives exp-147 inversion)"
    elif p_null:
        overall = "NULL — amplification arm is inert in GPT-2 medium"
    elif p1 and p2:
        overall = "CONFIRMED — amplification arm improves Task B in GPT-2 medium"
    elif p_positive:
    ...
    comparison = {
        "exp141_gpt2small_amplify_only_dB":   +0.27,
        "exp147_gpt2medium_combined_dB":       -0.13,
        "exp148_gpt2medium_amplify_only_dB":   dB_amp,
...
def main():
    print("exp-148 — World-model battery GPT-2 medium, amplify-only", flush=True)
    ...
    print(f"\n  K3: {n_amp_fail}/5 structural heads with κ̃_K < {K3_AMP_THRESHOLD} after amp",
          flush=True)
    if n_amp_fail >= 2:
        print("  WARNING: K3 fires — amplification may be insufficient.", flush=True)
    ...
    n_A_improved_amp  = sum(1 for o, c in zip(orig_A, amp_A)
                            if c["log_prob_correct"] > o["log_prob_correct"])
    n_B_improved_amp  = sum(1 for o, c in zip(orig_B, amp_B)
                            if c["log_prob_correct"] > o["log_prob_correct"])
    ...
    n_B_improved_sham = sum(1 for o, s in zip(orig_B, sham_B)
                            if s["log_prob_correct"] > o["log_prob_correct"])
    print(f"\n  Item-level (amp vs orig):  Task A={n_A_improved_amp}/20   Task B={n_B_improved_amp}/20",
          flush=True)
    print(f"  Item-level (sham vs orig): Task A={n_A_improved_sham}/20  Task B={n_B_improved_sham}/20",
          flush=True)

    print(f"\n  Comparison:", flush=True)
    print(f"    exp-141 (GPT-2 small, amplify-only, CONFIRMED):  ΔP_B = +0.27 nats", flush=True)
    print(f"    exp-147 (GPT-2 medium, combined, INCONCLUSIVE):  ΔP_B = −0.13 nats", flush=True)
    print(f"    exp-148 (GPT-2 medium, amplify-only):            ΔP_B = {d['amp_B']:+.2f} nats",
          flush=True)
    ...
    out = {
        ...
        "item_counts": {
            "n_A_improved_amp":  n_A_improved_amp,
            "n_B_improved_amp":  n_B_improved_amp,
