"""
exp-099 — Conformal Window Scan: C-alien-rich (Modal cloud launcher).
...
Corpus: C-alien-rich.bin (5 entity types, 6 stochastic rules, S=32 world states)
Seeds: 1900, 1901, 1902 (init seeds; data_seed 2900)

Phases:
  generate   : generate C-alien-rich corpus on Modal (~2-4h, CPU)
  seeds      : train + measure seeds 1900/1901/1902 in parallel
  control    : randomized-weights control on seed-1900 checkpoint
...
Usage (from repo root, ALWAYS use --detach for long runs):
    .venv/bin/python3 -m modal run --detach \\
        research/physics/experiments/exp-099_alien_rich/modal_exp099.py \\
        --phase all
...
# ─── Modal ─────────────────────────────────────────────────────────────────────
app = modal.App("exp099-alien-rich")

vol = modal.Volume.from_name("exp099-rich-data", create_if_missing=True)
...
image_run = (
    ...
    .add_local_file(str(SCRIPT_DIR / "gen_calien_rich.py"),
                    remote_path="/exp099/gen_calien_rich.py")
...
CORPUS    = "C-alien-rich.bin"
SEEDS     = {1900: "run_rich_s0", 1901: "run_rich_s1", 1902: "run_rich_s2"}
DATA_SEED = 2900
...
@app.function(
    image=image_run,
    gpu=None,
    timeout=14400,   # 4h: C-alien-rich generation is pure Python, no GPU
    volumes={"/data099": vol},
    memory=8192,
    retries=3,
)
def generate_corpus():
    """Generate C-alien-rich corpus on Modal (CPU, ~2-4h)."""
    import os
    output_path = f"/data099/{CORPUS}"
    if os.path.exists(output_path):
        print(f"generate SKIP — {output_path} already exists", flush=True)
        return
    _run_patched(
        "/exp099/gen_calien_rich.py",
        out_override="/data099",
        argv=["gen_calien_rich.py"],
    )
    vol.commit()
...
def seed_run(init_seed: int):
    """Train + measure one seed on C-alien-rich."""
    run_name    = SEEDS[init_seed]
    corpus_path = f"/data099/{CORPUS}"
    _run_patched(
        "/exp062/train.py",
        out_override="/data099",
        argv=[
    ...
    _run_patched(
        ...
        argv=[
            "measure.py",
            f"/data099/runs/{run_name}/step_2000",
            run_name,
            "--full-vocab",
        ],
    )
...
def control():
    """Randomized-weights control on seed-1900 checkpoint."""
    import shutil
    import torch
    from transformers import GPTNeoXForCausalLM

    run_name  = SEEDS[1900]
    ckpt_path = f"/data099/runs/{run_name}/step_2000"
    rand_path = f"/data099/runs/{run_name}/step_2000_randomized"

    model = GPTNeoXForCausalLM.from_pretrained(ckpt_path, torch_dtype=torch.float32)
    with torch.no_grad():
        g = torch.Generator(device="cpu").manual_seed(1900 + 1)
...
def collect_results() -> dict:
    ...
    for seed, run_name in SEEDS.items():
        p = Path(f"/data099/measurements/{run_name}.json")
        if not p.exists():
    ...
    ctrl_run = SEEDS[1900]
    p_ctrl = Path(f"/data099/measurements/{ctrl_run}_randcontrol.json")
    if p_ctrl.exists():
