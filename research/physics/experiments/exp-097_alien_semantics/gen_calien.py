"""
exp-097 — Alien-semantics corpus generator (C-alien).

Pre-registration: notes.md (committed before this script ran).

Generates a corpus of alien-world stories. Entities (Flurps, Blurns, Zarbs) have no
real-world semantic referents. Causal rules are arbitrary but rigorously maintained.
The simulator holds the world-state explicitly at every step.
...
Usage:
    python gen_calien.py [output_dir]

Output: <output_dir>/C-alien.bin — uint16 token IDs, same format as the series.
