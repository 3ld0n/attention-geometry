"""
exp-034 — GPT-2 temperature–BCFT sweep (REBUS × Junction 1 local test)

Hypothesis (pre-stated, notes/2026-05-26_rebus_junction1_formalization.md):
Increasing softmax temperature (lowering precision / inverse temperature) drifts
median Δ away from SYK 0.25 and weakens BCFT boundary parameter λ before the
power-law structure breaks.

Temperatures: T ∈ {0.5, 1.0, 2.0}. Reference T=1.0 → Δ_med ≈ 0.249 (exp-007).
...
def main() -> None:
    ...
    t05 = next(r for r in summary if r["temperature"] == 0.5)
    t2 = next(r for r in summary if r["temperature"] == 2.0)
    ...
    delta_monotone = None
    lambda_weakens = None
    if t05["delta_median"] and t1["delta_median"] and t2["delta_median"]:
        delta_monotone = bool(
            t05["delta_median"] <= t1["delta_median"] <= t2["delta_median"]
            or t05["delta_median"] >= t1["delta_median"] >= t2["delta_median"]
        )
    if t05["lambda_median"] is not None and t2["lambda_median"] is not None:
        lambda_weakens = bool(abs(t2["lambda_median"]) < abs(t05["lambda_median"]))

    verdict = "partial"
    if t1["delta_median"] and abs(t1["delta_median"] - 0.2493) < 0.02:
        if t2["delta_median"] and abs(t2["delta_median"] - t1["delta_median"]) > 0.01:
            verdict = "confirmed_drift"
        elif t2["n_power_law_heads_r2_gt_threshold"] < t1["n_power_law_heads_r2_gt_threshold"] * 0.8:
            verdict = "confirmed_structure_loss"
        else:
            verdict = "falsified_weak_effect"

    record = {
        "experiment": "exp-034",
        "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H%M%SZ"),
        "hypothesis": (
            "T↑ (lower precision) drifts Δ from 0.25 and weakens BCFT λ before power law breaks"
        ),
        "model": "gpt2",
        ...
        "interpretation": {
            "delta_monotone_with_T": delta_monotone,
            "lambda_abs_weakens_at_high_T": lambda_weakens,
            "verdict": verdict,
        },
