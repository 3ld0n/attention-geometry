"""
Phase Transition Width Scaling — Optimized Version

Focuses on the transition region (steps 64-2000) where the order parameter
jumps, with fewer checkpoints and smaller batches for larger models.
...
SEQ_LEN = 64  # shorter sequence for speed
N_INPUTS = 10  # fewer inputs for speed
MAX_DX = 40
MIN_POS = 16
...
def compute_locality_ratio_and_pl(model, n_layers, n_heads, vocab_size):
                ...
                for dx in range(MAX_DX):
                    ...
                    for i in range(max(MIN_POS, dx), seq):
                        ...
                        if j >= 0:
                            ...
                            cnt += 1
                    if cnt > 0:
                        pass  # accumulated, will normalize later
    ...
    return locality, pl_heads, near_quarter
...
MODELS = [
    ("EleutherAI/pythia-70m", "Pythia-70m", 6, 8, 48),
    ("EleutherAI/pythia-160m", "Pythia-160m", 12, 12, 144),
    ("EleutherAI/pythia-410m", "Pythia-410m", 24, 16, 384),
]

print("=" * 90)
print("  PHASE TRANSITION WIDTH SCALING (optimized)")
print("=" * 90)
print()

all_model_data = {}

for model_name, label, n_layers, n_heads, total_heads in MODELS:
    print(f"--- {label} ({total_heads} heads, {n_layers} layers) ---")
    ...
    for ckpt in TRANSITION_CHECKPOINTS:
        ...
        try:
            ...
            locality, pl, nq = compute_locality_ratio_and_pl(
                model, n_layers, n_heads, vocab_size
            )

            elapsed = time.time() - t0
            print(f"A(1)/A(32)={locality:.2f}, PL={pl}, near¼={nq} ({elapsed:.0f}s)")
            data.append({"step": step, "locality": locality, "pl": pl, "near_quarter": nq})
    ...
    # Summary
    print()
    print(f"  {'Step':>8}  {'A(1)/A(32)':>12}  {'PL':>5}  {'Near¼':>7}")
    print(f"  " + "-" * 40)
    for d in data:
        print(f"  {d['step']:>8}  {d['locality']:>12.2f}  {d['pl']:>5}  {d['near_quarter']:>7}")
    print()
...
for (_, label, _, _, total_heads), (_, model_data) in zip(MODELS, all_model_data.items()):
    if not model_data:
        continue

    # Find transition step: first step where A(1)/A(32) > 2
    ...
    for i, d in enumerate(model_data):
        if d["locality"] > 2.0 and transition_step is None:
            ...
            if i > 0:
                pre_transition_step = model_data[i-1]["step"]

    # Find when PL heads first appear
    first_pl_step = None
    for d in model_data:
        if d["pl"] > 0 and first_pl_step is None:
            first_pl_step = d["step"]

    print(f"  {label} (H={total_heads}):")
    ...
    if pre_transition_step is not None and transition_step is not None:
        # Transition happens between pre_transition_step and transition_step
        # Width in log-steps
...
print()
print("  KEY QUESTION: Does the transition become sharper with model size?")
print("  If so, this is consistent with the Hawking-Page transition scaling.")
print()
