#!/usr/bin/env python3
"""
exp-037 — Per-layer attention NaN fraction: fp16 vs fp32 on Pythia-410m (MPS).

Pre-stated: exp-036 found all-NaN attentions from L15+ under fp16 MPS. This probe
quantifies the onset layer and compares to the BCFT JSON truncation (L0–14 only).

One forward pass, seq_len=512, random tokens — same as BCFT protocol.
...
OUT = Path(__file__).resolve().parent / "fp16_nan_probe.json"
MODEL_NAME = "EleutherAI/pythia-410m"
SEQ_LEN = 512


def layer_nan_stats(attentions, n_layers: int, n_heads: int) -> list[dict]:
    rows = []
...
def probe(dtype: torch.dtype, device: torch.device) -> dict:
    ...
    rows = layer_nan_stats(outputs.attentions, n_layers, n_heads)
...
def main() -> None:
    ...
    bcft_json_last_layer = 14  # exp-030 / exp-036

    out = {
        "experiment": "exp-037_fp16_nan_probe",
        "date": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
        "model": MODEL_NAME,
        "seq_len": SEQ_LEN,
        "hypothesis": (
            "fp16 MPS attention goes all-NaN at layer L*; L* should align with "
            "BCFT JSON last valid layer (14) + 1."
        ),
        "fp16": fp16,
        "fp32": fp32,
        "alignment": {
            ...
            "layers_match": first_nan == bcft_json_last_layer + 1
            if first_nan is not None
            else None,
        },
        "verdict": (
            "confirmed"
            if first_nan == bcft_json_last_layer + 1
            else "partial"
        ),
    }
