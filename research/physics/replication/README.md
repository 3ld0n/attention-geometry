...
1. **Model census** (`measure_conformal_heads.py`) — one script, public
   checkpoints, minutes of runtime, no training. Does a trained model carry
   the conformal head population?
2. **Corpus census** (`census_corpus.py`) — train-small-and-measure. Does
   *your corpus* induce the structure that natural text does? One A100-hour,
   compared against the published formation ladder (`anchors.json`).
...
The corpus-formation result (pre-registered, three experiments): natural text
reliably induces the conformal population in a 70m model trained from scratch
(11–15/48 heads, three seeds), while corpora engineered to match natural
language's mutual-information statistics (0–5/48), a recursive PCFG with
hierarchy but no reference (0/48), and 1.1B tokens *generated by* the best
natural-text model (7/48 — while carrying **more** long-range MI than the
natural corpus) all fail the ≥ 10/48 formation criterion. Whatever the
geometry needs, it is not in the surface statistics — which makes the census
a screen that perplexity filters and MI profiles cannot replicate.

To run it on a candidate corpus (pretraining data, synthetic data, filtered
mixes):

```bash
# corpus: flat uint16 token ids, >= 1.05B tokens, any tokenizer w/ vocab <= 50304
python census_corpus.py my_corpus.bin --workdir census_run   # ~75 min on A100-40GB
```

This trains the frozen 70m protocol on your tokens (the *same* training and
measurement code that produced the anchors — executed from the experiment
directory, not a copy), censuses the step-2000 checkpoint, and prints your
corpus's position on the ladder. Anchors ship in `anchors.json`. To re-compare
an existing result without retraining:

```bash
python census_corpus.py --compare-only census_run/measurements/my_corpus.json
```

Worked example: the generated corpus of exp-085 lands at 7/48, below the
formation criterion, between the strongest engineered corpus (5/48) and the
natural-text band (11–15/48) — a first-generation structural collapse marker
that fired while the corpus's pairwise MI *exceeded* its parent's. Full
pre-registration and per-head data:
`../experiments/exp-085_generational_transmission/`.

Interpretation note: at the 70m/1B-token scale the observable is *formation*
(the ≥ 10/48 count), not SYK-exponent maturation — natural-text Δ_med at this
scale is ≈ 0.16–0.17, en route to but not at 0.25. Compare your `n_conformal`
to the ladder; do not expect Δ_med = 0.25 from a 70m model.
...
If you run this on a model family we have not measured — or the corpus census
on a dataset we should know about — we would like to see the JSON either way,
especially if it disagrees with the prediction.
