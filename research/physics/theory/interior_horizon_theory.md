...
MLP write is a **W_proj structure property**. **W_proj column structure (exp-134, September 6 2026 — CONFIRMED):** P1/P2/P3 all confirmed. The position-correlated structure of h_gelu is 2D (50% of position-variance in 2 components; 89% in first 100). W_proj amplification is carried by ≤2 output channels (ch480, ch87, σ_d=0.14) that route through this 2D subspace — not a dimensionality reduction artifact (σ(random projection)=0.126 ≈ σ(h_gelu)=0.121). Top-5 W_proj input singular directions applied to h_gelu give σ=0.424 > output σ=0.313 — W_proj's dominant directions route through a high-σ pocket of h_gelu; the output 0.313 is diluted by 766 near-zero channels. **h_gelu 2D subspace origin (exp-135, September 6 2026 — CONFIRMED):** The 2D structure is inherited from h^(0.5) (MLP input), not created by W_fc + GeLU. PCA dimensionality profile: h^(0) needs 1 component for 50% of position-variance (wpe is essentially 1D, PC1=53.86%); h^(0.5) needs 2 components (attention block broadens 1D → 2D; PC1=48.56%, PC2=18.93%); h_gelu also needs 2 (PC1=39.04%, PC2=18.01%). W_fc + GeLU preserve the 2D shape. **Level-3 chain fully closed: wpe 1D → attn_block0 broadens to 2D → h^(0.5) 2D → W_fc+GeLU preserve shape → h_gelu 2D → W_proj routes through ≤2 channels → σ≈Δ in mlp_out.** [LEVEL 3 FULLY TRACED.]
